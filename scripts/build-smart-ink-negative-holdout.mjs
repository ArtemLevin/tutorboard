...
function parseArguments(arguments_) {
  ...
  for (let index = 0; index < arguments_.length; index += 1) {
    ...
    if (argument === "--minimum-per-category") {
      minimumPerCategory = positiveInteger(
...
async function readJson(path) {
  ...
  if (Buffer.byteLength(content, "utf8") > maximumInputBytes) {
    throw new Error(
      `Negative holdout input expands beyond the size limit: ${path}`,
    );
  }
...
async function main(arguments_) {
  ...
  try {
    ...
    const result = smartInk.buildIndependentSmartInkNegativeHoldout(
      await merge(configuration.development),
      await merge(configuration.candidates),
      {
        minimumPerCategory: configuration.minimumPerCategory,
      ...
      `${JSON.stringify(
        {
          ...
          selectedCategoryCounts: result.selectedCategoryCounts,
