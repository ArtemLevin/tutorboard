...
PR 5 provides a user-facing editor session with an expected persisted definition, a live draft and transient selected-series state. The draft is rendered immediately through the Konva context. Save emits one stale-safe update command, so formula typing, slider motion and viewport gestures remain one semantic undo step. Escape closes the session while retaining the persisted object.

A plot can be created from the toolbar or shortcut `G`. Existing plots enter editing through double-click, Enter after selection or the selection inspector. The panel supports explicit and parametric series, domains and ranges, shared parameters, line styles, grid, axes, labels, legend and viewport controls. Expression diagnostics remain local to the affected fields and valid sibling series continue rendering.
...
Each sampled fragment becomes one independent Konva `Line`, preserving domain gaps and asymptotes. Series support solid, dashed and dash-dot strokes, opacity and width. A wider hit area makes thin curves selectable; selected or hovered series receive a halo while unrelated series are visually de-emphasized. Legend rows use the same selection contract. PR 5 connects the controlled selected-series and definition-override props to the editor session.

During internal editing, dragging inside the clipping rectangle pans the coordinate viewport. The wheel performs cursor-anchored zoom; Shift limits the operation to the horizontal range and Alt limits it to the vertical range. Board-object Transformer handles remain disabled for the session, keeping internal navigation separate from movement, rotation and scaling of the outer object.
...
Expression compilation remains outside BoardDocument validation. A malformed series can therefore be reopened and corrected while the document, unrelated objects and sibling series remain available. The editor reports expression diagnostics as advisory issues and reserves blocking validation for structural definition failures. The complete grammar and limits are specified in `COORDINATE_PLOT_EXPRESSION_LANGUAGE.md`; ADR-012 records the security and dependency decision.
...
The production renderer reuses one bounded cache across plot instances. Effective sampling zoom includes board zoom and object scale, so visual enlargement requests suitable detail and produces a distinct cache key. Aborted results are never cached. Fit-to-series derives a padded viewport from finite sampled data bounds. The full numerical algorithm and result contract are specified in `COORDINATE_PLOT_SAMPLING.md`; ADR-013 records the worker-compatibility decision. ADR-014 records the Konva rendering decision, and ADR-015 records the editor workflow.
...
Series selection belongs to the renderer/editor interaction layer and does not change BoardDocument, clipboard payloads or collaborative command history. Outer object selection continues to use the standard board selection model.

## Transfer compatibility

SVG and PNG board snapshots retain their existing deterministic contract. The production Konva renderer and editor affect the interactive canvas and do not change BoardDocument or transfer schemas. A future snapshot renderer can consume the same sampler output and grid helpers without introducing formula execution into serialization.

## Release verification

The release gate checks schema migration, nested domain limits, stale-safe updates, collaborative undo, Board sync transport, clipboard identity preservation, transformed marquee and lasso selection, renderer registration, expression grammar, contextual variables, evaluation domains, adaptive explicit and parametric sampling, discontinuity splitting, clipping, automatic grid steps, equal-scale resolution, line styles, legend placement, editor model operations, internal pan and zoom, sampled render geometry, cache behavior, contract freshness, browser smoke, production image hardening and security scanning.
